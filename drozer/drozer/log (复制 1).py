...
def analyze_log(log_file):
    ...
    with open(log_file, 'r', encoding='utf-8') as file:
        ...
        if "run app.activity.info" in log_content:
            if "Permission: null" in log_content:
                results["activity组件暴露"] = "通过"
                results["activity越权访问"] = "通过"
            else:
                results["activity组件暴露"] = "存在安全风险"
                results["activity越权访问"] = "手动测试"
        ...
        if "run app.broadcast.info" in log_content:
            if "Permission: null" in log_content:
                ...
                results["broadcast receiver组件暴露"] = "存在安全风险"
        ...
        if "run scanner.provider.finduris" in log_content:
            if "No respone from content URI" in log_content:
                results["content provider Uri"] = "通过"
            else:
                results["content provider Uri"] = "存在安全风险"
        ...
        if "run app.package.attacksurface" in log_content:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    if component in ["activities", "services", "broadcast receivers", "content providers"]:
                        results["组件导出暴露"] = "存在安全风险"
                        break
            else:
                results["组件导出暴露"] = "通过"
